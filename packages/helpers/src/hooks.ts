...
export type AsyncHooks = {
  [K in keyof Required<AstroLibs.Hooks>]: NonNullable<AstroLibs.Hooks[K]> extends (...params: any[]) => Promise<any> ? K : never;
}[keyof AstroLibs.Hooks];

export type SyncHooks = {
  [K in keyof Required<AstroLibs.Hooks>]: NonNullable<AstroLibs.Hooks[K]> extends (...params: any[]) => Promise<any> ? never : K;
}[keyof AstroLibs.Hooks];

export async function execLibHook<K extends AsyncHooks>(integrations: AstroIntegration[], hook: K, ...params: Parameters<AstroLibs.Hooks[K]>): Promise<void> {
...
export function execLibHookSync<K extends SyncHooks>(integrations: AstroIntegration[], hook: K, ...params: Parameters<AstroLibs.Hooks[K]>): void {
  for (const integration of integrations) {
    const libHooks = integration.libHooks;
    if (!libHooks) continue;
...
export interface PluginApi {
  [k: string]: unknown;
  execLibHook<K extends AsyncHooks>(hook: K, ...params: Parameters<AstroLibs.Hooks[K]>): Promise<void>;
