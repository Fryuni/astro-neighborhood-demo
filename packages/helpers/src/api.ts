...
export type AstroIntegrationSetupFn<Options extends z.ZodTypeAny, TApi = Record<string, never>> = (params: {
  name: string;
  options: z.output<Options>;
}) => Omit<AstroIntegration, "name"> & TApi;

export type IntegrationFactory<TOptionsSchema extends z.ZodTypeAny, TApi = Record<string, never>> = (
...
type AllHookPlugin<TName extends string, TApi> = {
  [Hook in keyof Required<AstroIntegration["hooks"]>]: (params: HookParameters<Hook>) => Record<TName, TApi | null>;
};
...
>(params: {
  ...
  const api: IntegrationApi<TName, TApi> = {
    is: (integration: any): integration is AstroIntegration & TApi => integration[integrationSymbol] === true,
    fromIntegrations: integrations => integrations.find(api.is) ?? null,
    fromConfig: config => api.fromIntegrations(config.integrations),
    asPlugin: <TAttr extends string>(attr: TAttr) => definePlugin({
      name: params.name,
      setup() {
